...
class CreateJyOrderTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('jy_order', function (Blueprint $table) {
            ...
            $table->tinyInteger('order_status')->default(0)->comment('订单状态:1 未确认  2 已确认  3 取消  4 退货');
            $table->tinyInteger('shipping_status')->default(0)->comment('发货状态:1 代发货  2 已发货  3 已收货  4 退货');
            $table->tinyInteger('pay_status')->default(0)->comment('支付状态:1 未付款  2 支付中  3 支付成功');
            ...
            $table->string('address')->default('')->comment('收货人的详细地区');
            $table->char('zcode',8)->default('')->comment('收货人的编号');
            $table->char('phone',11)->default('')->comment('收人的手机号');
